...
void factory(){
   ...
   Wire.write(0x20);                          //Calibration sequence byte 1
   Wire.endTransmission();
   delay(20);

   Wire.beginTransmission(ADDRESS);           //start communication with CMPS10
   Wire.write(0);                             //Send the register we wish to start reading from
   Wire.write(0x2A);                          //Calibration sequence byte 2
   Wire.endTransmission();
   delay(20);

   Wire.beginTransmission(ADDRESS);           //start communication with CMPS10
   Wire.write(0);                             //Send the register we wish to start reading from
   Wire.write(0x60);                          //Calibration sequence byte 2
   Wire.endTransmission();
   delay(20);
}
...
{
   //one.move(-10,10); // Slowly rotate the compass on the horizontal plane in all directions
   ...
   Wire.beginTransmission(ADDRESS);           //start communication with CMPS10
   Wire.write(0);                             //Send the register we wish to start reading from
   Wire.write(0xF6);                          //Calibration sequence byte 2
   Wire.endTransmission();
   delay(30);
